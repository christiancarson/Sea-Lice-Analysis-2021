...
Removing dates where >30 fish were caught. Turning dates into Julian to make it
easier to process with R. Establishing weekly intervals to group dates, samples, to take average from. 
...
```
Labelling the weekly interval with the given row. Empty weekly interval column is being filled out by this function. 
...
Subsets with forloops. It wants to calculate the abundance of each species at each site. It subsets for site, species, weekly interval, louse stage. It creates a weekly interval csv file for each site, as well as a plot for each site. 
```{r}
